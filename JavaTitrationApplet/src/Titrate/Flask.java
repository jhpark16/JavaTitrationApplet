...
class Flask extends Panel implements ActionListener, AdjustmentListener, ItemListener {
	...
	public Flask(Titrate applet) {
		...
		add(textLabel7);
		// textLabel8 = new Label("of the solution");
		// add(textLabel8);
		...
		textLabel5.setBounds(20 + ins.left, 100 + ins.top, 110, 20);
	 ...
	 * 100 milliseconds repaint(done ? 0 : 1000); return !done; //If done, no
	 * further updates required. }
	 */
	/*
	 * public boolean action(Event e, Object obje) { Object source = e.target;
	 * double f; if (source == but2) {
	 * ((CardLayout)applet.cards.getLayout()).next(applet.cards); return true; }
	 * else if (source == but1) {
	 ...
	 * slider.setValue((int)(f*20.0)); } catch (java.lang.NumberFormatException
	 * e2) { f = 0.2/20.0; } applet.chem.Mol1 = f; applet.chem.Molx =
	 * f+Math.random()*f/2.0-f/4.0; } else if (source == textfield2) { try { f =
	 * (double)Double.valueOf(textfield2.getText()).doubleValue();
	 * slider2.setValue((int)f); } catch (java.lang.NumberFormatException e2) {
	 * f = 25.0; } applet.chem.Vol1 = f; } else if (source==cb1) {
	 * applet.chem.K1 = 1e7; applet.chem.AcidFlag1 = true; } else if
	 * (source==cb2) { applet.chem.K1 = 20; applet.chem.AcidFlag1 = true; } else
	 * if (source==cb3) { applet.chem.K1 = 1.75e-5; applet.chem.AcidFlag1 =
	 * true; } else if (source==cb4) { applet.chem.K1 = 1.6;
	 * applet.chem.AcidFlag1 = false; } else if (source==cb5) { applet.chem.K1 =
	 * 3.15; applet.chem.AcidFlag1 = false; } else if (source==cb6) {
	 * applet.chem.K1 = 1.8e-5; applet.chem.AcidFlag1 = false; } else if
	 * (source==cb7) { applet.randflag = true; f =
	 * (double)Double.valueOf(textfield1.getText()).doubleValue();
	 * applet.chem.Mol1 = f; applet.chem.Molx = f+Math.random()*f/2.0-f/4.0; }
	 * else if (source==cb8) { applet.randflag = false; } else return
	 ...
	 * (double)Double.valueOf(textfield2.getText()).doubleValue();
	 * slider2.setValue((int)f); } catch (java.lang.NumberFormatException e2) {
	 * f = 25.0; } applet.chem.Vol1 = f; } else return super.lostFocus(e,obje);
	 * applet.chem.InitTitration(); return true; }
	 */
	/*
	 * public boolean handleEvent(Event e) { Object source = e.target; double f;
	 * if (source==slider) {
	 * textfield1.setText(String.valueOf(slider.getValue()/20.0)); f = (double)
	 * Double.valueOf(textfield1.getText()).doubleValue(); applet.chem.Mol1 = f;
	 * applet.chem.Molx = f+Math.random()*f/2.0-f/4.0; } else if
	 * (source==slider2) {
	 * textfield2.setText(String.valueOf(slider2.getValue()/1.0));
	 * applet.chem.Vol1 = (double)
